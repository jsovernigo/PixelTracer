...
void setpixelcolour(SDL_Surface *screen, int x, int y,colour c){
	Uint32 *pixel;
	Uint32 colour;
	int ybpp = y*screen->pitch / sc_bpp;
	colour = SDL_MapRGB(screen->format, c.r, c.g, c.b);//seems to be a method which converts rgb's using the screen's format.
	pixel = (Uint32*)screen->pixels + ybpp + x;//this can be used to CHECK PIXELS TOO
	*pixel = colour;
}
